...
const BlogStats = ({ stats }) => {
  return (
    <div className="flex gap-2 max-lg:mb-6 max-lg:pb-4 max-lg:border-b border-grey">
  ...
  return (
    <>
      <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
        ...
        />
        <div className="flex flex-col justify-between py-2 w-full mix-w-[300px]">
          <div>
            <Link
              to={`/blog/${blog_id}`}
...
export const ManageDraftBlogPost = ({ blog }) => {
  let { title, descrption, blog_id, index } = blog;
  let { userAuth: { access_token } = {} } = useContext(UserContext);
  index++;
    ...
    <div className="flex gap-5 lg:gap-10 pb-6 border-b mb-6 border-grey">
      ...
      <div>
        ...
        <p className="line-clamp-2 font-gelasio">
          {descrption.length ? descrption : "No Description"}
        </p>

        <div className="flex gap-6 mt-3">
          <Link to={`/editor/${blog_id}`} className="pr-4 underline py-2">
            Edit
          </Link>
          <button
            className="pr-4 py-2 underline text-red"
...
const deleteBlog = (blog, access_token, target) => {
  ...
  axios
    .post(
      import.meta.env.VITE_SERVER_DOMAIN + "/api/users/delete-blog",
      { blog_id },
      {
